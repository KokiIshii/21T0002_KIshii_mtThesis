...
sys_value = sys.argv #["", "grid-division", "hierarchical-data-structure", "leaf-size", "fluid-particles", , "rigid-body-frequency"]
#sys_value = ['', '136', 'bitmasked', '2', '{}'.format(11532*3),"1"]
...
rigid = [[0.00, 0.00, 0.25], [0.04, 0.50, 0.75]]
rxx = abs(np.array(rigid[1])-np.array(rigid[0]))
rxxl = (rxx/(l_size))
rn_particles = int(rxxl[0]*rxxl[1]*2 + rxxl[1]*rxxl[2]*2 + rxxl[2]*rxxl[0]*2 - rxxl[0]*4 - rxxl[1]*4 - rxxl[2]*4 + 8)
...
fre = float(sys_value[5]) #frequency

#boundary
bound = [[0, 0, 0.25], [1, 1, 0.75]]
...
@ti.kernel
def cal_rigid_xv(frame: ti.i32):
  #rigid_xv
  f = fre
  Rv[None] = ti.Vector([0.1 * 2 * math.pi * f * ti.sin(2 * math.pi * f * frame * dt), 0.0, 0.0])#
  Rx[None] += Rv[None] * dt
  for k in rx:
    rx[k] = Rx[None] + rx0[k]
    rv[k] = Rv[None]
...
def main():
  ...
  Rx[None] = [0.02, 0.25, 0.50]

  def setup_rigid_box_x():
    rx_l = []
    for i in range(int(rxxl[0])):
      for j in range(int(rxxl[1])):
        for k in range(int(rxxl[2])):
          if not(i == 0 or i == int(rxxl[0]) -1) and not(j == 0 or j == int(rxxl[1]) -1) and not(k == 0 or k == int(rxxl[2]) -1):
            pass
          else:
            rx_l.append([- rxx[0] / 2 + (i * l_size) + l_size,
                         - rxx[1] / 2 + (j * l_size) + l_size,
                         - rxx[2] / 2 + (k * l_size) + l_size])
    print("r_len:", len(rx_l))
    return np.array(rx_l)
  rx0.from_numpy(setup_rigid_box_x()) #placement with respect to 0
