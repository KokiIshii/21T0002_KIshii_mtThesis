...
sys_value = sys.argv #["", "grid-division", "hierarchical-data-structure", "leaf-size", "fluid-particles", "inflow-velocity"]
#sys_value = ['', '136', 'bitmasked', '2', '{}'.format(11532*5), 1.0]
...
#flow
flow_v0 = int(sys_value[5])
interval = 0.015/flow_v0
flow_particle_number = ti.field(ti.i32, shape=())
flow_particle_interval_time = ti.field(ti.f32, shape=())
...
pCFL[None] = 0
flow_particle_number[None] = 0
flow_particle_interval_time[None] = 0
...
def dda_particle(eye_pos, d):
  ...
  #fluid
  for k in range(flow_particle_number[None]):
    x = particle_x[k]
...
def initialize_particle_grid():
  for p in range(flow_particle_number[None]):#particle_x:
...
def cal_fluid_xv():
  ...
  for i in particle_x:
    ...
    if particle_x[i][0] + particle_v[i][0] * dt - l_size * 2  <= bound[0][0] or particle_x[i][0] + particle_v[i][0] * dt + l_size * 2 >= bound[1][0]:
      particle_v[i][0] = 0.0
    if particle_x[i][1] + particle_v[i][1] * dt - l_size * 2 <= bound[0][1]:# or particle_x[i][1] + particle_v[i][1] * dt >= bound[1][1]:
      particle_v[i][1] = 0.0
    if particle_x[i][2] + particle_v[i][2] * dt - l_size * 2 <= bound[0][2] or particle_x[i][2] + particle_v[i][2] * dt + l_size * 2 >= bound[1][2]:
      particle_v[i][2] = 0.0
...
def cal_rigid_xv():
  ...
  if Rx[None][0] + Rv[None][0] * dt - l_size * (inv_dx / 4) - rad <= 0.0 or Rx[None][0] + Rv[None][0] * dt + l_size * (
          inv_dx / 4) + rad >= 1.0:
    Rv[None][0] = 0.0
  if Rx[None][1] + Rv[None][
    1] * dt - l_size * 0 - rad <= 0.0:  
    Rv[None][1] = 0.0
  if Rx[None][2] + Rv[None][2] * dt - l_size * (inv_dx / 4) - rad <= 0.0 or Rx[None][2] + Rv[None][2] * dt + l_size * (
          inv_dx / 4) + rad >= 1.0:
    Rv[None][2] = 0.0
  Rom[None] += (Rtau[None] / (rp_rho * (rad * inv_dx) ** 2)) * dt
  Rx[None] += Rv[None] * dt
  Rthe[None] += Rom[None] * dt
  R = ti.Matrix([[ti.cos(Rthe[None][2]) * ti.cos(Rthe[None][1]),
                  ti.cos(Rthe[None][2]) * ti.sin(Rthe[None][1]) * ti.sin(Rthe[None][0]) - ti.sin(
                    Rthe[None][2]) * ti.cos(Rthe[None][0]),
                  ti.cos(Rthe[None][2]) * ti.sin(Rthe[None][1]) * ti.cos(Rthe[None][0]) + ti.sin(
                    Rthe[None][2]) * ti.sin(Rthe[None][0])],
                 [ti.sin(Rthe[None][2]) * ti.cos(Rthe[None][1]),
                  ti.sin(Rthe[None][2]) * ti.sin(Rthe[None][1]) * ti.sin(Rthe[None][0]) + ti.cos(
                    Rthe[None][2]) * ti.cos(Rthe[None][0]),
                  ti.sin(Rthe[None][2]) * ti.sin(Rthe[None][1]) * ti.cos(Rthe[None][0]) - ti.cos(
                    Rthe[None][2]) * ti.sin(Rthe[None][0])],
                 [-ti.sin(Rthe[None][1]), ti.cos(Rthe[None][1]) * ti.sin(Rthe[None][0]),
                  ti.cos(Rthe[None][1]) * ti.cos(Rthe[None][0])]])
  for k in rx:
...
@ti.kernel
def entry_flow():
  flow_particle_interval_time[None] += dt
  if flow_particle_interval_time[None] > interval and flow_particle_number[None] < num_part:
    flow_particle_interval_time[None] = 0
    n_size = int(inv_dx / 2.0)
    w = 0.20
    d = 0.20
    offset = [0.4, 0.8, 0.4]
    start = flow_particle_number[None]
    end = start + ((w*n_size)*(d*n_size))
    if end > num_part:
      end = num_part
    for i in range(start, end):
      particle_x[i] = [(i % (n_size * w)) / n_size + offset[0] - w / 2
        , offset[1]
        , (int(i / (n_size * w)) % (n_size * d)) / n_size + offset[2] - d / 2]
      particle_v[i] = [0.0, -flow_v0, 0.0]
      particle_color[i] = [0.5, 0.5, 1.0]
    flow_particle_number[None] = end
...
def main():
  ...
  def setup():
    for i in range(num_part):
      ...
      w = 0.40
      d = 0.39
      offset = [0.50, 0.02, 0.5]
      particle_x[i] = [(i % (n_size * w)) / n_size + offset[0]- w / 2
                       , int(i / (n_size * w * n_size * d)) / n_size + offset[1]
                       , (int(i / (n_size * w)) % (n_size * d)) / n_size + offset[2] - d / 2]
        ...
        for j in range(int(bxxl[1])):
          for k in range(int(bxxl[2])):
            if not(i == 0 or i == int(bxxl[0]) -1) and not(j == 0 or j == int(bxxl[1]) -1) and not(k == 0 or k == int(bxxl[2]) -1):
              ...
              bx_l.append([(bound[0][0] + bound[1][0]) / 2 - bxx[0] / 2 + (i * l_size) + l_size,
                           (bound[0][1] + bound[1][1]) / 2 - bxx[1] / 2 + (j * l_size) + l_size,
                           (bound[0][2] + bound[1][2]) / 2 - bxx[2] / 2 + (k * l_size) + l_size])
