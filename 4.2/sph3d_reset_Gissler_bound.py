...
#num_spheres = 1024 * 1024
num_part = 10647#11532#1024 * 8#
...
pid = ti.field(ti.i32)
pidroot = ti.root.pointer(ti.ijk, n_grid // 8).pointer(ti.ijk, 8)
pidroot.dynamic(ti.l, max_num_particles_per_cell, chunk).place(pid)

bpid = ti.field(ti.i32)
ti.root.pointer(ti.ijk, n_grid // 8).pointer(ti.ijk, 8).dynamic(ti.l, max_num_particles_per_cell, chunk).place(bpid)
    ...
    for i in range(-support, support + 1):
      for j in range(-support, support + 1):
        for k in range(-support, support + 1):
          offset = ti.Vector([i, j, k])
          box_ipos = ipos + offset
          ti.append(pid.parent(), box_ipos, p + num_part)
...
def main():
  num_particles[None] = num_part + rn_particles# + bn_particles##############################
  
  @ti.kernel
  def setup():
    for i in range(num_part):
      n_size = int(inv_dx / 2.0)
      #particle_x[i] = [ti.random() * 0.4 + 0.2, ti.random() * 0.4 + 0.2, ti.random() * 0.4 + 0.2]
      offset = [0.5, 0.4, 0.5]
  ...
  for frame in range(1000):
    ...
    #print(particle_a[0][0],particle_a[0][1],particle_a[0][2])
    if rendering == True:
      render()
