...
def cal_fluid_xv():
  ...
  for i in particle_x:
    ...
    if particle_x[i][0] + particle_v[i][0] * dt - l_size * (inv_dx / 4) <= 0.0 or particle_x[i][0] + particle_v[i][
      0] * dt + l_size * (inv_dx / 4) >= 1.0:
      particle_v[i][0] = 0.0
    if particle_x[i][1] + particle_v[i][
      1] * dt - l_size * 2 <= 0.0:
      particle_v[i][1] = 0.0
    if particle_x[i][2] + particle_v[i][2] * dt - l_size * (inv_dx / 4) <= 0.0 or particle_x[i][2] + particle_v[i][
      2] * dt + l_size * (inv_dx / 4) >= 1.0:
      particle_v[i][2] = 0.0
